...
struct LearnTab: View {
    ...
    private var gettingStartedSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            heroTopicCard(
                ...
                color: .orange,
                title: "How to Read TouchGrass BTC",
    ...
    private var referenceSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            heroTopicCard(
                ...
                color: .indigo,
                title: "Glossary",
                subtitle: "Key terms and definitions you'll encounter in TouchGrass BTC and beyond",
                lessonCount: 9
            )
        }
    }
    ...
    // MARK: - How to Read TouchGrass BTC

    private var howToReadEducation: some View {
        VStack(spacing: 0) {
            topicHeader(icon: "app.badge.checkmark", color: .orange, title: "How to Read TouchGrass BTC", subtitle: "Understand every feature of this app.")
            VStack(spacing: 12) {
    ...
    private var powerLawEducation: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 12) {
                ...
                formulaCard
                Text("Based on historical patterns. Past behavior does not guarantee future results. Where signal meets uncertainty.")
                    .font(.caption2)
                    .fontWeight(.semibold)
                    .foregroundStyle(.secondary)
                    .italic()
    ...
    private var cycleEducation: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 12) {
                ExpandableInfoCard(icon: "scissors", iconColor: .orange, title: "The Halving", summary: "A hard-coded event in Bitcoin's protocol", detail: "Every 210,000 blocks (~4 years), the mining reward is cut in half. Started at 50 BTC per block in 2009, now 3.125 BTC. Reduces the rate of new BTC entering circulation. Completely predictable — next one is ~April 2028. 4 halvings so far: 2012, 2016, 2020, 2024. A supply-side event — it's mechanical, not theoretical.")
                ExpandableInfoCard(icon: "square.stack.3d.up", iconColor: .cyan, title: "The Epoch (Era)", summary: "The period between two consecutive halvings", detail: "Epoch 1: Genesis → Nov 2012 (50 BTC reward). Epoch 2: Nov 2012 → Jul 2016 (25 BTC reward). Epoch 3: Jul 2016 → May 2020 (12.5 BTC reward). Epoch 4: May 2020 → Apr 2024 (6.25 BTC reward). Epoch 5: Apr 2024 → ~2028 (3.125 BTC reward). A time period defined by a fixed block reward. We're in Epoch 5.")
                ExpandableInfoCard(icon: "arrow.triangle.2.circlepath", iconColor: .purple, title: "The 4-Year Cycle", summary: "An observed market pattern, not a protocol rule", detail: "Bull tops and bear bottoms have formed ~4 years apart. Driven by halvings, monetary policy, and psychology. Each cycle: accumulation → bull → euphoria → bear. Only 4 complete cycles observed — small sample size. May weaken or end as Bitcoin matures. A theory based on historical observation — correlation, not causation.")
                ...
                ExpandableInfoCard(icon: "hammer.fill", iconColor: .orange, title: "Why Do 4-Year Cycles Happen?", summary: "Halvings, monetary policy, and investor psychology", detail: "Every ~4 years, mining rewards are cut in half — a programmatic supply shock. Each halving has coincided with the early stages of a new bull market. Crypto prices have tended to rise when the Fed cuts rates or injects liquidity, though this correlation doesn't always hold. The same boom-bust dynamics that drive most markets — optimism and greed push prices up, panic and fear push them down — play out over roughly 4-year intervals.")
                ...
                ExpandableInfoCard(icon: "app.badge", iconColor: .blue, title: "How TouchGrass BTC Uses It", summary: "One lens among many for understanding conditions", detail: "The cycle phase is combined with momentum, trend, positioning, and volatility to create a composite picture. It's a context tool — not a timing tool.")
            }
        }
        .transition(.asymmetric(insertion: .move(edge: .trailing).combined(with: .opacity), removal: .opacity))
    }
    ...
    private var glossaryEducation: some View {
        VStack(spacing: 0) {
            topicHeader(icon: "character.book.closed.fill", color: .indigo, title: "Glossary", subtitle: "Key terms you'll encounter in TouchGrass BTC and beyond.")
            VStack(spacing: 12) {
    ...
    private var explainBitcoinEducation: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 12) {
                ...
                trackedLesson(id: "explain_3", number: 3, color: .blue, icon: "doc.text.fill", title: "The Google Doc Analogy", summary: "Everyone can see the same document — no one can secretly edit it", detail: "Imagine a shared Google Doc that records every Bitcoin transaction ever made. Everyone in the world can see it. But here's the twist: nobody can edit past entries, nobody can delete rows, and nobody owns the document. That's Bitcoin's ledger. Instead of trusting a bank to keep accurate records, Bitcoin lets everyone verify the records together. If someone tries to cheat, thousands of computers instantly spot the discrepancy.")

                trackedLesson(id: "explain_4", number: 4, color: .purple, icon: "drop.fill", title: "The Water Meter Analogy", summary: "You can't fake how much water went through the pipe", detail: "Think of Bitcoin's public ledger like a water meter that every neighbor can read. You can see exactly how much water (money) flowed through every pipe (wallet). Nobody can claim they didn't receive water, and nobody can secretly add water. The meter is tamper-proof and public. That's why Bitcoin transactions are trustworthy without needing a bank to vouch for them.")
    ...
    private var psychologyEducation: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 12) {
                ...
                trackedLesson(id: "psych_5", number: 5, color: .cyan, icon: "shield.fill", title: "Conviction vs Gambling", summary: "Know the difference between informed belief and blind hope", detail: "Conviction comes from understanding: you've studied the technology, the economics, the history, and the math. Gambling comes from hoping the number goes up. Conviction lets you hold through a 50% drawdown because you understand the context. Gambling leads to panic selling at the bottom. TouchGrass BTC exists to build conviction through education — not to create false confidence. Know why you hold, and the volatility becomes manageable.")
            }
        }
        .transition(.asymmetric(insertion: .move(edge: .trailing).combined(with: .opacity), removal: .opacity))
    }
